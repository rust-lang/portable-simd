...
**Why you might not:** every version is more code to compile, test, and maintain. There's also a subtle performance boundary: a `#[target_feature]` function won't be inlined into a caller that doesn't enable the same features, so the call across that boundary is mostly opaque to the optimizer. That's the real reason to keep the dispatch *coarse* — choose the version once, high up, and do all the heavy work inside the chosen function, rather than crossing the boundary inside a hot inner loop. If the extra speed doesn't matter for your workload, a single portable version (or a compile-time `cfg` choice) is less to worry about.
